  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-[1.4fr_1fr] gap-x-12 gap-y-12 px-6 py-24 max-[860px]:grid-cols-1 max-[620px]:py-20">
        <div>
          <h2 className="max-w-[16ch] text-[clamp(2.2rem,4vw,3.4rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-[#0f172a]">
            {title}
          </h2>
          <p className="mt-5 max-w-[44ch] text-[1.05rem] leading-relaxed text-[#475569]">{desc}</p>
          <Link
            href={appHref}
            className="mt-8 inline-block rounded-full bg-[#4f46e5] px-8 py-3.5 text-base font-semibold text-white shadow-[0_8px_24px_-8px_rgba(79,70,229,0.6)] transition hover:-translate-y-px hover:bg-[#4338ca]"
          >
            {getStarted}
          </Link>
        </div>

        <div className="flex flex-col justify-center border-l border-[#e2e8f0] pl-12 max-[860px]:border-l-0 max-[860px]:border-t max-[860px]:pl-0 max-[860px]:pt-10">
          <span className="font-mono text-[12px] uppercase tracking-[0.18em] text-[#64748b]">
            {ossTitle}
          </span>
          <p className="mt-3 max-w-[36ch] text-[15px] leading-[1.6] text-[#475569]">{ossDesc}</p>
          <div className="mt-6 flex flex-col gap-3">
            <a
              href={REPO}
              target="_blank"
              rel="noopener noreferrer"
              className="group/link inline-flex w-fit items-center gap-2 text-sm text-[#0f172a] transition-colors hover:text-[#4f46e5]"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex w-fit items-center gap-2 text-sm text-[#475569] transition-colors hover:text-[#4f46e5]"
            >
