...
function FooterLink({
  ...
  const className =
    "group/link w-fit inline-flex items-center gap-1.5 text-sm text-[#475569] hover:text-[#4f46e5] transition-colors";
  const inner = (
    <>
    ...
    <footer className="relative isolate overflow-hidden bg-[#f6f7f9]">
      ...
      <div className="mx-auto grid max-w-[1200px] grid-cols-2 gap-x-8 gap-y-12 px-6 pb-44 pt-20 md:grid-cols-[1.7fr_1fr_1fr]">
        ...
        <div className="col-span-2 max-w-sm md:col-span-1">
          <Link
            href={`/${lang}`}
            className="flex w-fit items-center gap-2.5 text-2xl font-bold tracking-[0.02em] text-[#0f172a]"
          >
            <Logo className="h-7 w-7 text-[#0f172a]" />
            <span>Weftmap</span>
          </Link>
          <p className="mt-5 text-sm leading-relaxed text-[#475569]">{tagline}</p>
          <a
            href={REPO}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="GitHub repository"
            className="mt-7 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#e2e8f0] bg-white text-[#475569] transition-all hover:-translate-y-0.5 hover:border-[#c7d2fe] hover:text-[#4f46e5]"
          >
      ...
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 -bottom-[0.18em] -z-10 select-none whitespace-nowrap bg-gradient-to-t from-black/0 via-black/[0.03] to-black/[0.06] bg-clip-text text-center text-[20vw] font-bold leading-none tracking-tighter text-transparent"
      >
        Weftmap
      </span>

      <div className="relative border-t border-[#e2e8f0] bg-white/60 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1200px] flex-col items-center justify-between gap-2 px-6 py-6 text-xs text-[#64748b] sm:flex-row">
